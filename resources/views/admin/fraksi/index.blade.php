...
<div class="content-card">
    <div class="content-card-header" style="display: flex; justify-content: space-between; align-items: center;">
        <h2 class="content-card-title">Data Fraksi</h2>
        <a href="{{ route('admin.fraksi.create') }}" class="btn btn-primary" style="padding: 8px 16px; font-size: 14px;">
        ...
        <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 14px;">
            <thead>
                <tr style="background: #f9fafb; border-bottom: 2px solid #e5e7eb;">
                    ...
                    <th style="padding: 12px; color: #4b5563; font-weight: 600;">Nama Fraksi</th>
                    <th style="padding: 12px; color: #4b5563; font-weight: 600; width: 150px; text-align: center;">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @forelse($fraksi as $index => $item)
                <tr style="border-bottom: 1px solid #e5e7eb;">
                    <td style="padding: 12px; color: #374151;">{{ $index + 1 }}</td>
                    <td style="padding: 12px; color: #111827; font-weight: 500;">{{ $item->nama_fraksi }}</td>
                    <td style="padding: 12px; text-align: center;">
                        <a href="{{ route('admin.fraksi.edit', $item->id) }}" style="color: #3b82f6; margin-right: 12px; font-weight: 500; text-decoration: none;">Edit</a>
                        <form action="{{ route('admin.fraksi.destroy', $item->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus fraksi ini? Data yang terkait mungkin akan kehilangan relasinya.');">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="3" style="padding: 24px; text-align: center; color: #6b7280;">Belum ada data Fraksi.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
