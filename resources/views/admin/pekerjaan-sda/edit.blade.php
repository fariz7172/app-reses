...
<div class="content-card">
    ...
    <div style="padding: 24px;">
        ...
        <form action="{{ route('admin.pekerjaan-sda.update', $pekerjaan->eid) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                ...
                <div id="div_nama_dewan" style="{{ $pekerjaan->sumber_data == 'Masyarakat' ? 'display: none;' : '' }}">
                    ...
                    <select name="id_dewan" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                        ...
                        @foreach($dewans as $d)
                            <option value="{{ $d->id }}" {{ $pekerjaan->id_dewan == $d->id ? 'selected' : '' }}>{{ $d->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Kode Tracking</label>
                    <input type="text" name="kode_tracking" value="{{ old('kode_tracking', $pekerjaan->kode_tracking) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">No SKPD/UKPD</label>
                    <input type="text" name="no_skpd" value="{{ old('no_skpd', $pekerjaan->no_skpd) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Tahun Monev</label>
                    <input type="number" name="tahun_monev" value="{{ old('tahun_monev', $pekerjaan->tahun_monev) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Tanggal Input</label>
                    <input type="date" name="tgl_input" value="{{ old('tgl_input', $pekerjaan->tgl_input) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="id_kecamatan" id="select_kecamatan" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                        ...
                        @foreach($kecamatans as $kec)
                            <option value="{{ $kec->id }}" {{ $pekerjaan->id_kecamatan == $kec->id ? 'selected' : '' }}>{{ $kec->nama_kecamatan }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="id_kelurahan" id="select_kelurahan" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                        ...
                        @foreach($kelurahans as $kel)
                            <option value="{{ $kel->id }}" data-kec="{{ $kel->id_kecamatan }}" {{ $pekerjaan->id_kelurahan == $kel->id ? 'selected' : '' }}>{{ $kel->nama_kelurahan }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="alamat" rows="2" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">{{ old('alamat', $pekerjaan->alamat) }}</textarea>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <div style="display: flex; gap: 10px;">
                    <div style="flex: 1;">
                        ...
                        <input type="text" name="latitude" id="lat" value="{{ old('latitude', $pekerjaan->latitude) }}" readonly style="width: 100%; padding: 8px; border: 1px solid #d1d5db; border-radius: 6px; background: #f9fafb;">
                    </div>
                    <div style="flex: 1;">
                        <label style="display: block; font-size: 12px; color: #6b7280; margin-bottom: 4px;">Longitude</label>
                        <input type="text" name="longitude" id="lng" value="{{ old('longitude', $pekerjaan->longitude) }}" readonly style="width: 100%; padding: 8px; border: 1px solid #d1d5db; border-radius: 6px; background: #f9fafb;">
                    </div>
                </div>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="deskripsi" rows="3" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">{{ old('deskripsi', $pekerjaan->deskripsi) }}</textarea>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                <div>
                    ...
                    <input type="text" name="kategori_pekerjaan" value="{{ old('kategori_pekerjaan', $pekerjaan->kategori_pekerjaan) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Lingkup Kewenangan</label>
                    <input type="text" name="lingkup_kewenangan" value="{{ old('lingkup_kewenangan', $pekerjaan->lingkup_kewenangan) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Metode Pekerjaan</label>
                    <input type="text" name="metode_pekerjaan" value="{{ old('metode_pekerjaan', $pekerjaan->metode_pekerjaan) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Volume / Panjang (m / m&sup3;)</label>
                    <input type="text" name="volume_panjang" value="{{ old('volume_panjang', $pekerjaan->volume_panjang) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Progress Pekerjaan (%)</label>
                    <input type="number" name="progress" min="0" max="100" value="{{ old('progress', $pekerjaan->progress) }}" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; font-weight: bold; color: #059669;">
                </div>
                <div>
            ...
            <div style="margin-bottom: 30px;">
                ...
                <small style="color: #6b7280; margin-top: 4px; display: block;">*Foto yang diunggah akan ditambahkan ke foto sebelumnya.</small>
            </div>

            <div style="text-align: right;">
                <button type="submit" class="btn btn-primary" style="padding: 12px 24px; font-size: 16px;">Update Data Pekerjaan</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    window.onload = function() {
        ...
        if(kecSelect.value) {
            ...
            let kelIdServer = "{{ $pekerjaan->id_kelurahan }}";
            if(kelIdServer) {
