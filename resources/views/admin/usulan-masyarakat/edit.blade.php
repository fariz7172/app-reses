...
<div class="content-card">
    <div class="content-card-header">
        <h2 class="content-card-title">Edit Usulan Masyarakat</h2>
    ...
    <div style="padding: 24px;">
        ...
        <form action="{{ route('admin.usulan-masyarakat.update', $usulan->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                ...
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Status Usulan</label>
                    <select name="status" required style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px; font-weight: bold;">
                        <option value="Menunggu" {{ $usulan->status == 'Menunggu' ? 'selected' : '' }}>Menunggu</option>
                        <option value="Diproses" {{ $usulan->status == 'Diproses' ? 'selected' : '' }}>Diproses (Masuk Pekerjaan SDA)</option>
                        <option value="Ditolak" {{ $usulan->status == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                        <option value="Selesai" {{ $usulan->status == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="id_kecamatan" id="select_kecamatan" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                        ...
                        @foreach($kecamatans as $kec)
                            <option value="{{ $kec->id }}" {{ $usulan->id_kecamatan == $kec->id ? 'selected' : '' }}>{{ $kec->nama_kecamatan }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="id_kelurahan" id="select_kelurahan" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                        ...
                        @foreach($kelurahans as $kel)
                            <option value="{{ $kel->id }}" data-kec="{{ $kel->id_kecamatan }}" {{ $usulan->id_kelurahan == $kel->id ? 'selected' : '' }}>{{ $kel->nama_kelurahan }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="alamat" rows="2" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">{{ old('alamat', $usulan->alamat) }}</textarea>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <div style="display: flex; gap: 10px;">
                    <div style="flex: 1;">
                        ...
                        <input type="text" name="latitude" id="lat" value="{{ old('latitude', $usulan->latitude) }}" readonly style="width: 100%; padding: 8px; border: 1px solid #d1d5db; border-radius: 6px; background: #f9fafb;">
                    </div>
                    <div style="flex: 1;">
                        <label style="display: block; font-size: 12px; color: #6b7280; margin-bottom: 4px;">Longitude</label>
                        <input type="text" name="longitude" id="lng" value="{{ old('longitude', $usulan->longitude) }}" readonly style="width: 100%; padding: 8px; border: 1px solid #d1d5db; border-radius: 6px; background: #f9fafb;">
                    </div>
                </div>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="deskripsi_usulan" rows="4" required style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">{{ old('deskripsi_usulan', $usulan->deskripsi_usulan) }}</textarea>
            </div>
            ...
            @if($usulan->photo)
                ...
                @if(is_array($fotos) && count($fotos) > 0)
                    <div style="margin-bottom: 20px;">
                        <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 10px;">Foto Pendukung Saat Ini</label>
                        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                            @foreach($fotos as $index => $f)
                                <div style="position: relative; width: 120px; height: 120px; border-radius: 8px; overflow: hidden; border: 1px solid #d1d5db; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                    <img src="{{ Storage::url($f) }}" alt="Foto Usulan" style="width: 100%; height: 100%; object-fit: cover;">
            ...
            <div style="margin-bottom: 30px;">
                <label style="display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px;">Upload Foto Pendukung Tambahan (Bisa lebih dari 1)</label>
            ...
            <div style="text-align: right;">
                <button type="submit" class="btn btn-primary" style="padding: 12px 24px; font-size: 16px;">Update Usulan</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    window.onload = function() {
        ...
        if(kecSelect.value) {
            ...
            let kelIdServer = "{{ $usulan->id_kelurahan }}";
            if(kelIdServer) {
